...
def save_colleges():
    ...
    response=requests.get(URL) # Rendering the URL
    college_soup=BeautifulSoup(response.text) # Parsing with BeautifulSoup
    rows=college_soup.table.find_all('tr')[1:] # Slicing the head row
...
def roll_no_generator(college_code, year):
    ...
    branch_codes = ['80', '89', '35', '81', '65', '50', '74', '52', '54', '64',
    '88', '53', '55', '51', '00', '15', '12', '11', '10', '21', '20', '33',
    '31', '32', '30', '34', '82', '45', '16', '13', '22', '23', '86', '62',
    '41', '14', '70', '72', '43', '40', '42', '07', '84', '85', '87', '46',
    '44', '83', '60', '63', '61']
...
def get_result_data(roll_no, year):
    ...
    if year >= 3:
        rows_all = { 'result_odd': tables[3].find_all('tr')[1:],
        'result_even': tables[5].find_all('tr')[1:]
        }
    else:
        rows_all = { 'result_odd': tables[2].find_all('tr')[3:],
            ...
            if subject_code != '':
                ...
                if year <= 2:
                    ...
                    try:
                        ...
                        marks_ps = 0
                    try:
                        if year==1: credit = int(cols[7].text)
                        else: credit=int(cols[6].text)
                    except ValueError:
                        credit = 0
                else:
                    marks_pe=0
    ...
    for roll_no in roll_nos:
        ...
        while not_found < 5:
            ...
            while get_result_data(roll_no, year):
                ...
                if data['student_details']['branch_code'] not in [branch.code \
                ...
                if int(data['student_details']['roll_no']) not in \
                    [student.roll_no for student in Student.objects.all()]:
                    new_student = Student(
                        roll_no = roll_no,
                    ...
                    for key in ['result_odd', 'result_even']:
                        for subject_code in data[key]:
                            if subject_code not in [subject.code for subject in \
                                Subject.objects.all()]:
                                new_subject = Subject(
